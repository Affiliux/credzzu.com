    ...
    <Dialog open={isOpen} onOpenChange={open => !open && onClose()}>
      <DialogContent className='rounded-xl border border-emerald-500/20 bg-black/80 p-0 shadow-xl backdrop-blur-sm sm:max-w-[400px]'>
        ...
        <DialogFooter className='flex flex-row gap-2 p-6 pt-2'>
          <Button variant='outline' className='h-10' onClick={onClose} disabled={loading}>
            Voltar
          </Button>
          <Button className='h-10 bg-red-500 text-white hover:bg-red-600' onClick={handleConfirm} disabled={loading}>
