 ...
 * @category Http - Services - Subscription - Get Subscription
 *
 ...
 * @category Http - Services - Subscription - Cancel Subscription
 *
...
export async function update_subscription(payload: UpdateSubscriptionPayloadProps): Promise<void> {
  try {
    const { data: response } = await api.patch('subscription/update-card', payload)
    return response
  } catch (error: AxiosError | any) {
    throw new Error(error?.response.status)
  }
 ...
 * @category Http - Services - Subscription - Reactivate Subscription
 *
 * @param {ReactivateSubscriptionPayloadProps} payload - The payload of the request
 * @return {PromiseLike<void>} - The response of the API
 *
 */

export async function reactivate_subscription(payload: ReactivateSubscriptionPayloadProps): Promise<void> {
  try {
    const { data: response } = await api.patch(`subscription/reactivate/${payload.id}`, payload)
    return response
  } catch (error: AxiosError | any) {
    throw new Error(error?.response.status)
  }
